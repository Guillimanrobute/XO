...
function initCellElementList() {
    ...
    if (ulElement) {
        ulElement.addEventListener('click', (eventIn) => {
            if (eventIn.target.tagName !== 'LI') return;
            const index = Number.parseInt(eventIn.target.dataset.idx)
            handleCellClick(eventIn.target, index)
        })
    }
}
